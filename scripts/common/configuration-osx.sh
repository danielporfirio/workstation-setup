...
# Set computer name "MacPorfa" (as done via System Preferences → Sharing)
sudo scutil --set ComputerName "MacPorfa"
sudo scutil --set HostName "MacPorfa"
sudo scutil --set LocalHostName "MacPorfa"
sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string "MacPorfa"
...
# Use keyboard keys (F1, F2 ...) as default
...
# Disable iTunes auto-launch when connected to a device
...
defaults write com.apple.screencapture location -string "${HOME}/Documnets/Screenshots"
...
set -u
# modify appearance of dock: remove standard icons, add Notes, Spotify, chrome and iTerm
